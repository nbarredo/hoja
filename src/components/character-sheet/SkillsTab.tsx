...
export function SkillsTab({ characterData }: SkillsTabProps) {
  ...
  characterData.skills?.forEach(skill => {
    characterSkills.set(skill.name.toLowerCase(), skill.value)
  })
  ...
  const allSkillsWithBonuses = allSkills.map(skill => {
    ...
    // Otherwise calculate base ability modifier
    const skillBonus = characterSkill !== undefined ? characterSkill : abilityModifier
    const isProficient = characterSkill !== undefined
